...
const EducationViewCompnent = ({ educationDetail, index }) => {
  ...
  const deleteDetail = () => {
    const { deleteEducationDetail } = educationContext;
    deleteEducationDetail(educationDetail, index);
  };
      ...
      >
        <div className="grid grid-cols-2 gap-3">
          ...
          <UiTextInfo label={"Since"}>
            {formatDate(educationDetail?.since, DATE_FORMAT)}
          </UiTextInfo>
          <UiTextInfo label={"Until"}>
            {formatDate(educationDetail?.until, DATE_FORMAT)}
          </UiTextInfo>
